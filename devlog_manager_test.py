...
class DevLogManagerFunctionalityTester(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        print("\n=== TESTING MANAGER FUNCTIONALITY IN DEVLOG APPLICATION ===\n")
        print("Using sample data with the following credentials:")
        print("- Manager: sarah_manager / Demo123!")
        print("- Developers: john_dev, alice_dev, bob_dev, emma_dev (all with password Demo123!)")
        print("\nRunning tests for the following requirements:")
        print("1. Manager login with sarah_manager/Demo123!")
        print("2. Manager access to /api/team/developers endpoint")
        print("3. Manager access to /api/team/logs endpoint")
        print("4. Filtering options on team logs (by date, by developer_id)")
        print("5. Adding feedback to developer logs")
        print("6. Verifying developers are correctly assigned to the manager")
        print("\n" + "="*60 + "\n")
    ...
    def test_01_manager_login(self):
        ...
        print(f"✅ User details: {data['user']['username']} (ID: {data['user']['id']}, Role: {data['user']['role']})")
    
    def test_02_access_team_developers(self):
        """Test manager access to team developers endpoint"""
        print("\n=== TEST 2: Access to /api/team/developers Endpoint ===")
    ...
    def test_03_access_team_logs(self):
        ...
        print("\n=== TEST 3: Access to /api/team/logs Endpoint ===")
        headers = {"Authorization": f"Bearer {self.manager_token}"}
    ...
    def test_04_filter_team_logs_by_date(self):
        ...
        print("\n=== TEST 4: Filtering Team Logs by Date ===")
        headers = {"Authorization": f"Bearer {self.manager_token}"}
    ...
    def test_05_filter_team_logs_by_developer(self):
        ...
        print("\n=== TEST 5: Filtering Team Logs by Developer ID ===")
        headers = {"Authorization": f"Bearer {self.manager_token}"}
    ...
    def test_06_add_feedback_to_developer_logs(self):
        ...
        print("\n=== TEST 6: Adding Feedback to Developer Logs ===")
        headers = {"Authorization": f"Bearer {self.manager_token}"}
    ...
    def test_07_verify_developer_assignment(self):
        ...
        print("\n=== TEST 7: Verifying Developer Assignment to Manager ===")
        headers = {"Authorization": f"Bearer {self.manager_token}"}
    ...
    def test_08_developer_cannot_access_manager_endpoints(self):
        """Test that developers cannot access manager-only endpoints"""
        print("\n=== TEST 8: Role-Based Access Control ===")
        
        # Test for each developer
        for username in self.dev_usernames:
            if username not in self.dev_tokens:
                print(f"⚠️ Skipping access control test for {username} - login failed")
                continue
                
            headers = {"Authorization": f"Bearer {self.dev_tokens[username]}"}
            
            # Try to access team developers
            response = requests.get(f"{self.base_url}/team/developers", headers=headers)
            self.assertEqual(response.status_code, 403, f"{username} should not access team developers list")
            
            # Try to access team logs
            response = requests.get(f"{self.base_url}/team/logs", headers=headers)
            self.assertEqual(response.status_code, 403, f"{username} should not access team logs")
            ...
            response = requests.post(f"{self.base_url}/feedback", json=feedback_data, headers=headers)
            self.assertEqual(response.status_code, 403, f"{username} should not be able to add feedback")
            
            print(f"✅ Role-based access control prevents {username} from accessing manager endpoints")
